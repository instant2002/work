...
<table class="table table-striped">
	...
	<c:forEach var="paymentList" items="${paymentList}" varStatus="status">
		<tr>
			<td><img src="${pageContext.request.contextPath}${paymentList.book_img_storedName}" style="width: 80px;"></td>
			<td style="vertical-align: middle;">
				<a href="/book/detailView.do?product_no=${paymentList.product_no }">${paymentList.book_name}</a>
			</td>
			<td style="vertical-align: middle;">${paymentList.quantity}</td>
			<td style="vertical-align: middle;"><fmt:formatNumber value="${paymentList.origin_price*paymentList.quantity}" groupingUsed="true"/>원</td>
			<td style="vertical-align: middle;"><fmt:formatNumber value="${paymentList.dc_price*paymentList.quantity}" groupingUsed="true"/>원</td>
...
<table class="table payInfo-table" style="margin-top: 50px; text-align: center;">
	<tr>
		<td colspan="5" class="total-head">결제 정보</td>
	</tr>
	<colgroup>
		<col style="width: 20%">
		<col style="width: 20%">
	...
	<tr>
		<td>
		<c:choose>
			<c:when test="${resultMap.payMethod == 'VCard'}">
				카드				
			</c:when>
			<c:otherwise>
				${resultMap.payMethod}
...
<div class="col-md-5"></div>
	<div class="col-md-2">
		<button class="btn btn-buy-col pull-left add-cart" onclick="location.href='/customer/orderList.do'" style="width: 100%; margin-bottom: 100px;">
